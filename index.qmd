...
Good, consistent visualization will map different dimensions of the data to different encoding channel types using appropriate graphical marks [@wilke2019fundamentals, chapter 2]. 
...
[TODO: THAT'S MORE REGRESSION THAN ACCURACY]
...
Multiplication can assess the interaction of the two orthogonal measures.
...
incorporating additional information about covariate-outcome importance.
...
Therefore, outcome-informed Love plot can help a fuller picture, 
...
In scatter plots, the adjusted and unadjusted ASMD are plotted on the x and y axes.
In slope graphs, the pre- post adjustment indication is mapped to the x-axis snd the ASMD to the y-axis.
Scatter plots are better suited for high-dimensional settings where plotting all covariates on the y-axis axis is infeasible (or at least too crowded). 
Slope graphs have a similar strength, compromising some information richness in exchange of emphasizing overall reduction in ASMD. 
Both variants can benefit by the same augmenting encoding (@fig-oui-bal-plot). 


#### Scale-less relative ranking
The main limitation of the outcome-informed ASMD score is that it losses scale. 
ASMD has a clear scale in standard deviation units.
It can both be interpreted as Cohen's d and is very well-established, so thresholds---although arbitrary---are still fairly understood and agreed on.
The outcome-informed ASMD score, on the other hand, being a multiply of the ASMD, no longer enjoys the familiarity and interpretability of the original ASMD, as it has no absolute scale.
Therefore, its utility is only in its ranking, comparing covariates relative to other covariates in the same analytic dataset. 


#### Augmenting Love plot with outcome-informed ASMD
Another small limitation of the Love plot augmented by covariate-outcome importance is that
it may over emphasize prognostic variables.
Those may still be differentiated by having small ASMD (because they have no covariate-exposure association), thus still forcing the researcher to be more mindful looking for confounding factors.
The immediate solution is thus to use the combined outcome-informed ASMD score. 
The limitation of them being scale-less is irrelevant because the visual use is just about ranking confounding variables above prognostic variables, which the outcome-informed ASMD score still fulfills faithfully. 
@fig-ouiasmd-love shows the difference between outcome-informed Love plot and Love plots augmented by outcome-informed ASMD score.
